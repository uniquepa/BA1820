...
ggplot(data, aes(x = x, y = fx)) + geom_line() + geom_bar(stat="identity", colour="#FF9999") 
```
...
ggplot(data, aes(x = x, y = fx)) + geom_line() + geom_bar(stat="identity", colour="#FF3465") 
```
...
fx <-  2 * x ^ 2 - 10
data <- data.frame(x = x, fx = fx)
ggplot(data, aes(x = x, y = fx)) + geom_line(colour="#FF1054") + geom_bar(stat="identity", colour="#FF1278") 
```
...
ggplot(data = cd, aes(x = x)) + 
       geom_line(aes(y = cos),color="#FF3489") + geom_line(aes(y = cos_prime),color="#FF7645")
```
...
ggplot(data = cd2, aes(x = x, y = value, linetype = variable)) + 
       geom_line(size = 2,colour="#FF7609")
```
